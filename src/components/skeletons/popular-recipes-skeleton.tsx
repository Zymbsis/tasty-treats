...
const range = Array.from({ length: 4 });
      ...
      <div className="flex h-[158px] flex-col gap-6 overflow-hidden md:h-[316px] md:gap-4 xl:h-[364px] xl:gap-8">
        {range.map((_, index) => (
          <div key={index} className="flex h-[67px] gap-4 md:gap-2 xl:gap-4">
            <Skeleton className="size-16 shrink-0 rounded-sm md:size-12 xl:size-16" />
            <div className="flex w-full flex-col gap-2">
              <Skeleton className="h-[18px] rounded-sm" />
              <Skeleton className="h-[41px] rounded-sm" />
            </div>
          </div>
        ))}
      </div>
